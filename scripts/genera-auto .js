...
/* Destinazioni: le auto vanno SOLO negli annunci di veicoli. Senza queste
   due colonne Merchant Center le tratta anche come prodotti Shopping, e le
   auto in Shopping violano le norme (avviso sull'account, revisione
   infinita). Nel nuovo Merchant Center la destinazione non si sceglie
   dall'interfaccia dell'origine: si dichiara qui, riga per riga. */
const FEED_COLS=['id','VIN','google_product_category','store_code','title','brand','model','trim','year','mileage','price','condition','color','engine','date_first_registered','image_link','additional_image_link','link','link_template','description','included_destination','excluded_destination'];
function rigaFeed(a){
  const url=CONFIG.BASE_URL+'/'+CONFIG.DIR_AUTO+'/'+slug(a)+'.html';
  ...
  return [
    ...
    tsv(desc),
    'vehicle_ads',
    'Shopping_ads,Free_listings,Display_ads,Free_local_listings,Local_inventory_ads'
  ].join('\t');
}
